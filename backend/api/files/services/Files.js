...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Files
      .findOne(_.pick(params, _.keys(Files.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count files.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Files.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Files.associations.map(a => a.alias));
    const data = _.omit(values, Files.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Files.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Files.associations.map(async association => {
